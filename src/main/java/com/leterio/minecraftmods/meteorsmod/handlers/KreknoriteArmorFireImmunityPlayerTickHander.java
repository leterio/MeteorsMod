...
@EventBusSubscriber
public class KreknoriteArmorFireImmunityPlayerTickHander {
	@SubscribeEvent
	public static void onPlayerTick(PlayerTickEvent event) {
		...
		// Just for speed
		if (heldItems[0] != null && heldItems[0].getItem() instanceof ItemKreknoriteArmor
	...
	private static void updateImmunity(Entity entity, boolean is) {
		if (entity.isImmuneToFire() != is) {
			try {
				...
				Logger.error(() -> "Fatal error ocurred when setting fire immunity. Please, open a issue on github. Thanks.", e);
			}
		}
	}
